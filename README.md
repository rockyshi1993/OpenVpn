...
3. **重启 OpenVPN 服务**：重新启动 OpenVPN 服务，适用于配置更改后或服务异常时。

4. **查看服务器信息**：显示 OpenVPN 服务器的详细信息，包括：
   - 服务状态和运行时间
   - 服务器 IP 地址
   - 配置信息（协议、端口、子网、加密算法等）
   - 防火墙状态
...
5. **查看 VPN 实时状态**：实时监控 VPN 连接状态，包括：
   - 已连接的客户端列表
...
6. **完全卸载 OpenVPN**：完全删除 OpenVPN 及其所有相关文件和配置，包括：
   - 删除 OpenVPN 软件包
   - 删除配置文件和证书
   - 删除日志文件
   - 移除防火墙规则
